...
class AnglesConverter : public rclcpp::Node {
    public:
        AnglesConverter() : Node("angles_conversion_service") {
            euler_to_quaternion_ = create_service<arduinobot_msgs::srv::EulerToQuaternion>("euler_to_quaternion_", std::bind(&AnglesConverter::eulerToQuaternionCallback, this, _1, _2));
            quaternion_to_euler_ = create_service<arduinobot_msgs::srv::QuaternionToEuler>("quaternion_to_euler_", std::bind(&AnglesConverter::quaternionToEulerCallback, this, _1, _2));
    ...
    private:
        ...
        void quaternionToEulerCallback(const std::shared_ptr<arduinobot_msgs::srv::QuaternionToEuler::Request> req,
            ...
            RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Request to convert quaternion x: " << req->x << ", y: " << req->y << ", z: " << req->z << ", w: " << req->w << " into a euler");
